...
std::vector<CiphertextT> det_clone_0_0(
  ...
  std::vector<size_t> v6 = {21, 22, 51, 52};
...
#pragma omp parallel for
  for (auto v20 = 0; v20 < 4; ++v20) {
    size_t v22 = v6[v20];
    const auto& ct21 = cc->EvalFastRotation(
        ct19, v22, 2 * cc->GetRingDimension(), digit_decomp1);
    const std::vector<CiphertextT> v23 = {ct21};
    v18[v20] = v23[0];
  }
  const auto& ct22 = v18[0];
  const auto& ct23 = v18[1];
  auto ct24 = v18[2];
  const auto& ct25 = v18[3];
  cc->EvalSubInPlace(ct24, ct25);
  cc->EvalAddInPlace(ct24, ct22);
  std::vector<CiphertextT> v24(1);
  cc->EvalSubInPlace(ct24, ct23);
  std::vector<CiphertextT> v25(v24);
  v25[0] = ct24;
  return v25;
}
